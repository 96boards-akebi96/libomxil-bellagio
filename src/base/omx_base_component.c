...
OSCL_EXPORT_REF OSCL_EXPORT_REF OMX_ERRORTYPE omx_base_component_GetParameter(
  ...
  //Avoid the warnings:
  //nParamIndex holds OMX_INDEXTYPE or OMX_INDEXVENDORTYPE...
...
void* compMessageHandlerFunction(void* param) {
  ...
  // 2015/06/23 shunji@iots-2 add Null pointer check
  ...
  while(1){
    ...
    // 2015/06/23 shunji@iots-2 add Null pointer check
    if (omx_base_component_Private->messageSem == NULL) {
      break;
    }
  ...
  */
  ...
  case OMX_CommandStateSet: {
    ...
    if (err != OMX_ErrorNone) {
      ...
      if (omx_base_component_Private->callbacks->EventHandler) {
        break;
      }
